...
- ### 语言  
    ...
    而`Node.JS`通常是前端开发人员在进行`单元测试`时使用。
...
- ### 工具库  
    ...
    - 早期的UI自动化工具，以`Selenium`、`Robot Framework`为例：  
        ...
        > 它们无法直接与浏览器进行通讯，往往要借助第三方驱动如：WebDriver。也因为这样，它们的运行效率要低于新一代工具，并且使用门槛要高于新一代工具。
        <span style="color: rgb(198, 202, 205)">PS：Web Driver安装确实挺麻烦的，Chrome浏览器更新了之后，如果还没有对应的Web Driver版本，则需要回退Chrome或者下载对应的Chrome离线安装包。</span>
    ...
    <table border="1">
        ...
        <tbody>
            <tr>
                ...
                <td>没有内置的图像比较功能，没有自带的测试报告功能，需要安装和配置浏览器驱动，需要处理一些浏览器和网页的兼容性问题。安装流程复杂，需要同时兼顾浏览器与WebDriver版本。</td>
            </tr>
            <tr>
                <td>Robot Framework</td>
