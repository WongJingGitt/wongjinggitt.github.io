...
- ## 面向对象的核心概念
    ...
    - ### 多态
        ...
        多态性可以简单理解为不同类提供了一个相同的函数来实现某些功能。无论对象的具体类型如何，只要它们都实现了相同的方法或属性，就可以通过相同的方式进行调用或操作。
        ...
        1. 首先我定义了一个基类`MusicDownload`：
            ...
            这里需要考虑的一件事是，我是从不同的网站中爬取歌曲，不同的网站他们的接口、爬虫代码肯定是不一样的。但是不管哪个网站，我要做的事情都是一样的：搜索、返回搜索结果、从结果中选择歌曲、下载。  
