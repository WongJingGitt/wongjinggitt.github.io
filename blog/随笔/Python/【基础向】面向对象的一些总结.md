...
- ## 面向对象的核心概念
    ...
    - ### 继承
        ...
        1. 父类与子类：被继承的类通常被成为父类，如果要科学一点的话通常也叫做“基类”或“超类”。而继承的类通常成为之类
        ...
        首先定义了一个父类 `Father`，以及两个子类 `Son` 和 `Daughter`，父类包含了一些通用的属性和方法，而子类继承了父类的这些特征。例如，`Son` 和 `Daughter` 类继承了 `Father` 类中的 `GENDER`、`name`、`__age`、`country` 和 `status` 属性， 继承了 `go_to_work` 和 `off_work` 方法。

        并且可以根据需要进行拓展和覆写。例如：`Son` 和 `Daughter` 类覆写了父类中的 `go_to_work` 和 `off_work` 方法，输出了不同的信息。
